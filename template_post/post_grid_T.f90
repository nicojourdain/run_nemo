...
&          sbt_ID, sbs_ID, deptht_bounds_ID, deptht_ID, fidM, fidSBC,    &
...
REAL*4,ALLOCATABLE,DIMENSION(:,:) :: ISOMIP_isf_draft, ISOMIP_bathy
...
file_in_T   = 'MONTHLY/ISOMIP-CCCC_monthly_YYYY_grid_T.nc'
file_in_SBC = 'MONTHLY/ISOMIP-CCCC_monthly_YYYY_SBC.nc'
file_MSH    = '../../input/nemo_ISOMIP/mesh_mask_ISOMIP_EEEE.nc'
file_out    = 'MONTHLY/ISOMIP-CCCC_monthly_YYYY_tmpT.nc'
...
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"convection","Enhanced vertical diffusivity")
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
status = NF90_PUT_VAR(fidM,salinityYZ_ID,ISOMIP_SYZ)
call erreur(status,.TRUE.,"var_salinityYZ_ID")
status = NF90_PUT_VAR(fidM,temperatureYZ_ID,ISOMIP_TYZ)
call erreur(status,.TRUE.,"var_temperatureYZ_ID")
status = NF90_PUT_VAR(fidM,salinityXZ_ID,ISOMIP_SXZ)
call erreur(status,.TRUE.,"var_salinityXZ_ID")
status = NF90_PUT_VAR(fidM,temperatureXZ_ID,ISOMIP_TXZ)
call erreur(status,.TRUE.,"var_temperatureXZ_ID")
status = NF90_PUT_VAR(fidM,bottomSalinity_ID,ISOMIP_Sbot)
call erreur(status,.TRUE.,"var_bottomSalinity_ID")
status = NF90_PUT_VAR(fidM,bottomTemperature_ID,ISOMIP_Tbot)
call erreur(status,.TRUE.,"var_bottomTemperature_ID")
status = NF90_PUT_VAR(fidM,halineDriving_ID,ISOMIP_halin_driv)
call erreur(status,.TRUE.,"var_halineDriving_ID")
status = NF90_PUT_VAR(fidM,thermalDriving_ID,ISOMIP_therm_driv)
call erreur(status,.TRUE.,"var_thermalDriving_ID")
status = NF90_PUT_VAR(fidM,frictionVelocity_ID,ISOMIP_ustar)
call erreur(status,.TRUE.,"var_frictionVelocity_ID")
status = NF90_PUT_VAR(fidM,meltRate_ID,ISOMIP_melt_rate)
call erreur(status,.TRUE.,"var_meltRate_ID")
status = NF90_PUT_VAR(fidM,bathymetry_ID,ISOMIP_bathy)
call erreur(status,.TRUE.,"var_bathymetry_ID")
status = NF90_PUT_VAR(fidM,iceDraft_ID,ISOMIP_isf_draft)
