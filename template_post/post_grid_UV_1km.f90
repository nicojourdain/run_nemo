...
&          time_ID, utbl_ID, uoce_e3u_ID, vtbl_ID, voce_e3v_ID, depthu_bounds_ID, cnt, nx_CRS, ny_CRS,      &
...
&                                      ISOMIP_vTBL, CRS_ISOMIP_BSF, CRS_ISOMIP_MOC, CRS_ISOMIP_uTBL, CRS_ISOMIP_vTBL
...
file_in_U  = 'MONTHLY/ISOMIP1km-CCCC_monthly_YYYY_grid_U.nc'
file_in_V  = 'MONTHLY/ISOMIP1km-CCCC_monthly_YYYY_grid_V.nc'
file_out   = 'MONTHLY/ISOMIP1km-CCCC_monthly_YYYY_tmpUV.nc'
  ...
  do i=2,mx-1
    ...
    do j=2,my-1
      if ( misf(i,j,1) .le. 1 ) then
        ...
        ISOMIP_BSF (i-1,j-1,l) = 0.25 * ( psif(i,j) + psif(i-1,j) + psif(i,j-1) + psif(i-1,j-1) )
      else
...
!---------------------------------------
! Average on coarser ISOMIP+ grid :

nx_CRS=(mx-2)/2
ny_CRS=(my-2)/2

ALLOCATE( CRS_ISOMIP_BSF(nx_CRS,ny_CRS   ,mtime_counter) )
ALLOCATE( CRS_ISOMIP_MOC(nx_CRS,  mznew,mtime_counter) )
ALLOCATE( CRS_ISOMIP_uTBL(nx_CRS,ny_CRS,mtime_counter), CRS_ISOMIP_vTBL(nx_CRS,ny_CRS,mtime_counter) )

CRS_ISOMIP_BSF=0.e0
CRS_ISOMIP_MOC=0.e0
CRS_ISOMIP_uTBL=0.e0
CRS_ISOMIP_vTBL=0.e0
...
do l=1,mtime_counter
  ...
  do k=1,mznew
    ...
    cnt=0
    if ( ISOMIP_MOC(2*i-1,k,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_MOC(i,k,l) = CRS_ISOMIP_MOC(i,k,l) + ISOMIP_MOC(2*i-1,k,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_MOC(2*i,k,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_MOC(i,k,l) = CRS_ISOMIP_MOC(i,k,l) + ISOMIP_MOC(2*i,k,l)
      cnt=cnt+1
    endif
    if ( cnt .ne. 0 ) then
      CRS_ISOMIP_MOC(i,k,l) = CRS_ISOMIP_MOC(i,k,l) / cnt
    else
      CRS_ISOMIP_MOC(i,k,l) = NF90_FILL_FLOAT
    endif
  ...
  do j=1,ny_CRS
    ...
    cnt=0
    if ( ISOMIP_BSF(2*i-1,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_BSF(i,j,l) = CRS_ISOMIP_BSF(i,j,l) + ISOMIP_BSF(2*i-1,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_BSF(2*i-1,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_BSF(i,j,l) = CRS_ISOMIP_BSF(i,j,l) + ISOMIP_BSF(2*i-1,2*j,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_BSF(2*i,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_BSF(i,j,l) = CRS_ISOMIP_BSF(i,j,l) + ISOMIP_BSF(2*i,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_BSF(2*i,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_BSF(i,j,l) = CRS_ISOMIP_BSF(i,j,l) + ISOMIP_BSF(2*i,2*j,l)
      cnt=cnt+1
    endif
    if ( cnt .ne. 0 ) then
      CRS_ISOMIP_BSF(i,j,l) = CRS_ISOMIP_BSF(i,j,l) / cnt
    else
      CRS_ISOMIP_BSF(i,j,l) = NF90_FILL_FLOAT
    endif

    cnt=0
    if ( ISOMIP_uTBL(2*i-1,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_uTBL(i,j,l) = CRS_ISOMIP_uTBL(i,j,l) + ISOMIP_uTBL(2*i-1,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_uTBL(2*i-1,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_uTBL(i,j,l) = CRS_ISOMIP_uTBL(i,j,l) + ISOMIP_uTBL(2*i-1,2*j,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_uTBL(2*i,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_uTBL(i,j,l) = CRS_ISOMIP_uTBL(i,j,l) + ISOMIP_uTBL(2*i,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_uTBL(2*i,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_uTBL(i,j,l) = CRS_ISOMIP_uTBL(i,j,l) + ISOMIP_uTBL(2*i,2*j,l)
      cnt=cnt+1
    endif
    if ( cnt .ne. 0 ) then
      CRS_ISOMIP_uTBL(i,j,l) = CRS_ISOMIP_uTBL(i,j,l) / cnt
    else
      CRS_ISOMIP_uTBL(i,j,l) = NF90_FILL_FLOAT
    endif

    cnt=0
    if ( ISOMIP_vTBL(2*i-1,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_vTBL(i,j,l) = CRS_ISOMIP_vTBL(i,j,l) + ISOMIP_vTBL(2*i-1,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_vTBL(2*i-1,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_vTBL(i,j,l) = CRS_ISOMIP_vTBL(i,j,l) + ISOMIP_vTBL(2*i-1,2*j,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_vTBL(2*i,2*j-1,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_vTBL(i,j,l) = CRS_ISOMIP_vTBL(i,j,l) + ISOMIP_vTBL(2*i,2*j-1,l)
      cnt=cnt+1
    endif
    if ( ISOMIP_vTBL(2*i,2*j,l) .ne. NF90_FILL_FLOAT ) then
      CRS_ISOMIP_vTBL(i,j,l) = CRS_ISOMIP_vTBL(i,j,l) + ISOMIP_vTBL(2*i,2*j,l)
      cnt=cnt+1
    endif
    if ( cnt .ne. 0 ) then
      CRS_ISOMIP_vTBL(i,j,l) = CRS_ISOMIP_vTBL(i,j,l) / cnt
    else
      CRS_ISOMIP_vTBL(i,j,l) = NF90_FILL_FLOAT
    endif
...
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"vertical_diffusion","Constant eddy diffusivity and viscosity (100 m2/s)")
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
status = NF90_PUT_VAR(fidM,utbl_ID,CRS_ISOMIP_uTBL)
call erreur(status,.TRUE.,"var_utbl_ID")
status = NF90_PUT_VAR(fidM,vtbl_ID,CRS_ISOMIP_vTBL)
call erreur(status,.TRUE.,"var_vtbl_ID")
status = NF90_PUT_VAR(fidM,BSF_ID,CRS_ISOMIP_BSF)
call erreur(status,.TRUE.,"var_BSF_ID")
status = NF90_PUT_VAR(fidM,MOC_ID,CRS_ISOMIP_MOC)
call erreur(status,.TRUE.,"var_MOC_ID")
status = NF90_PUT_VAR(fidM,z_ID,znew)
call erreur(status,.TRUE.,"var_z_ID")
status = NF90_PUT_VAR(fidM,x_ID,0.500000*(glamt(2:mx-2:2,1,1)+glamt(3:mx-1:2,1,1))*1.e3)
call erreur(status,.TRUE.,"var_x_ID")
status = NF90_PUT_VAR(fidM,y_ID,0.500000*(gphit(1,2:my-2:2,1)+gphit(1,3:my-1:2,1))*1.e3)
