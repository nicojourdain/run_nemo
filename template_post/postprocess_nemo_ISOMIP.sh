...
sed -e "s/YYYY/${YEAR}/g; s/CCCC/${CASE}/g ; s/EEEE/${EXP}/g ; s/GGGG/${GAMMAT}/g ; s/SSSS/${GAMMAS}/g" post_grid_UV.f90 > trpuv_${YEAR}.f90
ifort -c $NC_INC trpuv_${YEAR}.f90
ifort -o trpuv_${YEAR} trpuv_${YEAR}.o $NC_LIB
./trpuv_${YEAR}
if [ -f MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpUV.nc ]; then
  echo "MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpUV.nc  [oK]"
  rm -f trpuv_${YEAR} trpuv_${YEAR}.o trpuv_${YEAR}.f90
else
  echo "~!@#$%^&* Creation of MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpUV.nc failed >>>>>> stop !"
  exit
fi

sed -e "s/YYYY/${YEAR}/g; s/CCCC/${CASE}/g ; s/EEEE/${EXP}/g ; s/GGGG/${GAMMAT}/g ; s/SSSS/${GAMMAS}/g" post_grid_T.f90 > trpt_${YEAR}.f90
ifort -c $NC_INC trpt_${YEAR}.f90
ifort -o trpt_${YEAR} trpt_${YEAR}.o $NC_LIB
./trpt_${YEAR}
if [ -f MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpT.nc ]; then
  echo "MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpT.nc  [oK]"
  rm -f trpt_${YEAR} trpt_${YEAR}.o trpt_${YEAR}.f90
else
  echo "~!@#$%^&* Creation of MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpT.nc failed >>>>>> stop !"
  exit
fi

ncks -A  MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpUV.nc  MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpT.nc
rm -f MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpUV.nc
mv MONTHLY/ISOMIP-${CASE}_monthly_${YEAR}_tmpT.nc MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc
ncatted -O -h -a history,global,d,, -a NCO,global,d,, -a nco_openmp_thread_number,global,d,, MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc
