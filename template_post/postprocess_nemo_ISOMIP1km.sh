...
rm -f mesh_hgr.nc mesh_zgr.nc
ln -s -v ../../input/nemo_${CONFIG}/mesh_mask_${CONFIG}_${EXP}.nc mesh_hgr.nc
ln -s -v ../../input/nemo_${CONFIG}/mesh_mask_${CONFIG}_${EXP}.nc mesh_zgr.nc
...
sed -e "s/YYYY/${YEAR}/g; s/CCCC/${CASE}/g ; s/EEEE/${EXP}/g ; s/GGGG/${GAMMAT}/g ; s/SSSS/${GAMMAS}/g" post_grid_UV_1km.f90 > trpuv_${YEAR}.f90
ifort -c $NC_INC trpuv_${YEAR}.f90
ifort -o trpuv_${YEAR} trpuv_${YEAR}.o $NC_LIB
./trpuv_${YEAR}
if [ -f MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpUV.nc ]; then
  echo "MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpUV.nc  [oK]"
  rm -f trpuv_${YEAR} trpuv_${YEAR}.o trpuv_${YEAR}.f90
else
  echo "~!@#$%^&* Creation of MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpUV.nc failed >>>>>> stop !"
  exit
fi

sed -e "s/YYYY/${YEAR}/g; s/CCCC/${CASE}/g ; s/EEEE/${EXP}/g ; s/GGGG/${GAMMAT}/g ; s/SSSS/${GAMMAS}/g" post_grid_T_1km.f90 > trpt_${YEAR}.f90
ifort -c $NC_INC trpt_${YEAR}.f90
ifort -o trpt_${YEAR} trpt_${YEAR}.o $NC_LIB
./trpt_${YEAR}
if [ -f MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpT.nc ]; then
  echo "MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpT.nc  [oK]"
  rm -f trpt_${YEAR} trpt_${YEAR}.o trpt_${YEAR}.f90
else
  echo "~!@#$%^&* Creation of MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpT.nc failed >>>>>> stop !"
  exit
fi

ncks -A  MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpUV.nc  MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpT.nc
rm -f MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpUV.nc
mv MONTHLY/ISOMIP1km-${CASE}_monthly_${YEAR}_tmpT.nc MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc
ncatted -O -h -a original_config,global,c,c,"$CONFIG" -a original_case,global,c,c,"$CASE" -a history,global,d,, -a NCO,global,d,, -a nco_openmp_thread_number,global,d,, MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc MONTHLY/${CASE}_COM_NEMO-CNRS_y${YEAR}.nc
