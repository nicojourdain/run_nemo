      ...
      if ( status .eq. 0 ) then
        ...
        if ( status .eq. 0 ) then
          ...
          if ( status .eq. 0 ) then
            ...
            status = NF90_INQ_VARID(fidA,"time_average_3h",time_average_ID)
            if ( status .eq. 0 ) then
              freq = "3h" ! as in file name
              stat_tmin = NF90_INQ_VARID(fidA,"time_maximum_3h",time_maximum_ID)
              stat_tmax = NF90_INQ_VARID(fidA,"time_minimum_3h",time_minimum_ID)
            else
              ...
              write(*,*) '         ONLY 1d, 3d, 5d, 1h, 3h ARE SUPPORTED SO FAR...'
              write(*,*) '                                         >>>>>>>>>>>>>>>>>>>>>>>>> STOP !!!!'
              stop
            endif
          endif
